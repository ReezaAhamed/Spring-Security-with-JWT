...
import com.reeza.springsecurityandjwt.filter.CustomAuthorizationFilter;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private final Algorithm tokenAlgorithm; // beran created in AlgorithmForToken class
	...
	protected void configure(HttpSecurity http) throws Exception { //spring by defaullt uses the security system based on session which is stateful, tracking the suer by giving a cookie, we don't want that. we need jason web token system
		...
		CustomAuthenticationFilter customAuthenticationFiler = new CustomAuthenticationFilter(authenticationManagerBean(), tokenAlgorithm);
		...
		http.authorizeRequests().antMatchers("api/token/refresh/**").permitAll(); // order matters
		...
		http.authorizeRequests().antMatchers(HttpMethod.POST, "/api/role/**").hasAuthority("ROLE_ADMIN"); //to save a role or add role to employee (**), one must have the role admin
		...
		http.addFilterBefore(new CustomAuthorizationFilter(tokenAlgorithm), UsernamePasswordAuthenticationFilter.class); // we need to tell why CustomAuthorizationFilter for -> which is for the specific class UsernamePasswordAuthenticationFilter
	}
